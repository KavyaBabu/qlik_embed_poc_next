...
const QlikTable: React.FC<QlikTableProps> = ({
  ...
  return (
    ...
    >
      {(isHovered || showMenu) && (
        <div ref={menuRef} className="menu-button" style={{ position: 'absolute', top: '10px', right: '10px' }}>
          <button
          ...
          <Table.Header>
            <Table.Row>
              {columnConfigs.map((config) => {
                const showSortOption = ['projectName', 'projectNumber', 'startDate', 'endDate', 'plannedEndDate', 'actualEndMMMYYY', 'plannedEndMMMYYY', 'closedMMMYYY'].includes(config.key);
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      {showSortOption && (
                        <Table.SortBy
                          ...
                          selected={sortConfig.key === config.key && sortConfig.direction ? sortConfig.direction : undefined}
                        />
                      )}
                    </div>
                  </Table.Head>
                        ...
                        <Table.Cell key={colIndex} className="qlik-table-cell">
                          ...
                          {cellValue && cellValue.length > 100 && (
                            <div style={{ marginTop: '4px' }}>
                              <Button variant="text" onClick={() => toggleExpand(globalIndex)} size="sm">
