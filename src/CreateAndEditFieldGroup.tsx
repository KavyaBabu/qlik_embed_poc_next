...
function CreateAndEditGroupFieldGroup({
  ...
  // Create FieldGroup once to avoid remounting (which reset local state on file parse)
  const FieldGroup = useMemo(() => withFieldGroup<CreateGroupFields, CreateGroupFields>({
    render: ({ group }: any) => {
      ...
      const handleFileParse = (parsed: any) => {
        ...
        setFileUploadedMeterIds((prev) => {
          ...
          // Also update the form with the merged set and current dropdown selections
          updateFormMeterIds(next, dropdownSelectedMeterIds);
          return next;
        });
      };
        const raw = (parsed.data as ParsedRow[])
