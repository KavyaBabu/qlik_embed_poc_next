...
function CreateAndEditGroupFieldGroup({
  ...
  // 🔒 Keep the FieldGroup component identity stable across renders to prevent remounts
  const FieldGroupRef = useRef<any>(null);
  if (!FieldGroupRef.current) {
    FieldGroupRef.current = withFieldGroup<CreateGroupFields, CreateGroupFields>({
      render: ({ group }: any) => {
        ...
        const handleFileParse = (parsed: any) => {
          ...
          // Uniques within the file
          const newUniqueFileIds = new Set(raw);

          // Deduplicate across existing group + current dropdown selection
          const alreadyChosen = union(
            dropdownSelectedMeterIds,
            isEditMode ? existingSnapshotRef.current : new Set()
          );
          ...
          // Merge with previously uploaded file IDs using a functional update to avoid stale closures
          setFileUploadedMeterIds((prev) => {
  const next = union(prev, new Set(trulyNew));
  setDropdownSelectedMeterIds((prevDropdown) => {
    // Preserve dropdown selection exactly as-is
    updateFormMeterIds(next, prevDropdown);
    return prevDropdown;
  });
  return next;
});
